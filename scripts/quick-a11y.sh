...
LOG_DIR="${LOG_DIR:-/tmp/astronautui-logs}"   # <-- outside repo
...
wait_for_storybook() {
  ...
  for i in {1..60}; do
    if curl -sSf "${SB_URL}/iframe.html" >/dev/null 2>&1; then
      echo "Storybook is up."
      return 0
    fi
    sleep 2; printf "."
  done
  echo; echo "ERROR: Storybook not available at ${SB_URL}"; exit 1
}

RUNNER_CMD="pnpm exec test-storybook --url ${SB_URL} --maxWorkers=50%"
